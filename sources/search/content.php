...
if ($pt->config->total_videos > 1000000) {
    $get_videos = $db->rawQuery("SELECT * FROM " . T_VIDEOS . " WHERE MATCH (title) AGAINST ('$keyword') AND privacy = 0 ORDER BY id ASC LIMIT 20");
} else {
    $get_videos = $db->rawQuery("SELECT * FROM " . T_VIDEOS . " WHERE title LIKE '%$keyword%' AND privacy = 0 ORDER BY id ASC LIMIT 20");
}
